...
CREATE OR REPLACE FUNCTION calcular_complejo_result_cache(x NUMBER) 
...
SELECT /*+ PARALLEL(4) */ 
       id, calcular_complejo_result_cache(valor) as resultado
FROM numeros_prueba
...
SELECT calcular_complejo_result_cache(100) from dual;
SELECT calcular_complejo_result_cache(100) from dual; -- Más rápido
SELECT calcular_complejo_result_cache(100) from dual; -- Aún más rápido
--------------------------------------------------------------------
-- Ver si se está usando paralelismo
EXPLAIN PLAN FOR
SELECT /*+ PARALLEL(4) */ 
       id, calcular_complejo_result_cache(valor)
FROM numeros_prueba WHERE id <= 10;
